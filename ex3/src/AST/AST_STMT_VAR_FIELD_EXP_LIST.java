...
public class AST_STMT_VAR_FIELD_EXP_LIST extends AST_STMT
{
	...
	public AST_STMT_VAR_FIELD_EXP_LIST(AST_VAR var, int varLine, int varColumn, String fieldName, int fieldNameLine, int fieldNameColumn, AST_EXP_LIST el)
	{
	...
	public TYPE SemantMe()
	{
		...
		TYPE_LIST func_iter = null, calling_params = null, calling_params_iter = null;

		if (var != null) typeVar = var.SemantMe();

		/* STEP 1: making sure var is of a class type */
		...
		{	
			/* Iterating over cFieldList, could be function, could be var */
			if (it.head.name.equals(fieldName))
			{	
				/* it.head is a cField we need it's type */
				if(it.head.type.isFunction() == false)
				{
					System.out.format(">> ERROR [%d:%d] class instance (%s) has no function (%s)\n", varLine, varColumn, var.name, fieldName);
					SYMBOL_TABLE.getInstance().printERROR(varLine);
					System.exit(0);
				}
					...
					else
					{
						...
						else
						{
							...
							if(calling_params_iter == null && func_iter == null)
							{
								...
								System.out.format(">> ERROR [%d:%d] method %s.%s has unmatching signature - TOOMANY/TOOFEW parameters.\n",varLine, varColumn, var.name,
								var.name, fieldName);
